...
std::pair<Vec2f, float> Node::GetEdge() const {
...
void Node::_TickDfs(float food, float elapsedSec) {
    ...
    if (_parent) {
        auto [edge, angle] = _parent->GetEdge();
        _position = edge;
        _angle = angle;
    }
    ...
    for (SPtr& child : _children) {
        child->_TickDfs(foodPart, elapsedSec);
    }
}
...
void NodeSeed::Tick(float&/* food*/, float elapsedSec) {
    float food = 0;
    if (_foodStorage > 0) {
        food += std::min(_foodStorage, _phenotype->GetSeedFoodStorageConsumption());
...
void NodeSprout::Tick(float& food, float elapsedSec) {
    ...
    if (_active && _foodAccumulated > _phenotype->GetSproutAccumulatedToGrow()) {
        ...
        _children.emplace_back(new NodeBranch(this, _position, _angle + deltaAngle, _phenotype));
...
{
    auto [pos, a] = GetEdge();
    _children.emplace_back(new NodeSprout(this, pos, a, _phenotype));
}
